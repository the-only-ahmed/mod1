...
float *getVoxelColor(float r, float g, float b, float a)
{
...
void drawVox(Vector3 p, int flag, int type) {
	...
	if (type == 4)
		color = getVoxelColor(1, 0, 0, 1);
	else if (type == 5)
		color = getVoxelColor(0.2f, 0.2f, 0.2f, 1);
	else
	{
		if (flag >= 0 && flag < 3)
			color = getVoxelColor(1, 1, 1, 0.5f);
		else if (flag >= 3 && flag <= 30)
				color = getVoxelColor(0, 0.2, 0.7, 0.4f);
		else
			color = getVoxelColor(0, 0, 1, 0.7f);
	}
		...
		for(int z=0; z<16; z++)
		{
			for(int y=0; y<yMax; y++)
			{
				for(int x=0; x<xMax; x++)
				{
					...
					if (type >= 3)
					{
						int flag2 = wat._MW[z][y][x].getPerc();
						drawVox(wat._MW[z][y][x].getV3(), flag2, type);
					}
				}
			}
		}
